...
TCB* kCreateTask(QWORD qwFlags, void* pvMemoryAddress, QWORD qwMemorySize, QWORD qwEntryPointAddress)
{
	...
	//태스크 id로 스택 어드레스 계산, 하위 32비트가 스택 ㅜㄹ의 어프셋 역할 수행
...
static void kSetUpTask(TCB* pstTCB, QWORD qwFlags, QWORD qwEntryPointAddress, void* pvStackAddress, QWORD qwStackSize)
{
	...
	//Retrun Address 영역에 kExitTask() 함수의 어드레스를 삽입하여 태스크의
//엔트리 포인트 함수를 빠져나감과 동시에 kExitTask() 함수로 이동하도록 함
	*(QWORD *)( ( QWORD) pvStackAddress + qwStackSize - 8 ) = ( QWORD) kExitTask;
	...
	//RFlag 레지스터의 if비트를 1로 설정하여 인터럽트 활성화(비트9)
...
static TCB* kGetNextTaskToRun(void)
{
	...
	for (j = 0; j < 2; j++)
	{
		//높은 우선순위에서 낮은 우선수누이까지 리스트를 확인하여 스케줄링할 태스크 선택
		for (i = 0; i < TASK_MAXREADYLISTCOUNT; i++)
		{
			iTaskCount = kGetListCount(&(gs_stScheduler.vstReadyList[i]));

			//만약 실행한 횟수보다 리스트의 태스크 수가 더많으면 현재 우선순위의 태스크를 실행함
			...
			//만약 실행한 횟수가 더 많으면 실행 횟수를 ㅗ기화하고 다음 우선순위로 양보함
			else
			{
				gs_stScheduler.viExecuteCount[i] = 0;
			}
...
void kSchedule(void)
{
	...
	//전환하는 도중 인터럽트가 발생하여 태스크 전환이 도 일어나면 곤란하므로
	//전환하는 동안 인터럽트가 방생하지 못하도록 설정
	//임계 영역 시작
	bPreviousFlag = kLockForSystemData();
...
//현재 수행 중인 태스크의 정보를 수정한 뒤 콘텍스트 전환
	...
	//유휴 태스크에서 전환되었다면 사용한 프로세서 시간을 증가시키
	...
	//태스크 조욜 플래그가 설정된 경우 콘텍스트를 저장할 필요가 없으므로 대기 리스트
	//에 삽입하고 콘텍스트 전환
...
BOOL kScheduleInInterrupt(void)
{
 ...
 //=============================================== ======================
	...
	//태스크가 종료되지 않으면 ist에 있는 콘텍스트를 복사하고현재 태스크를 준비 리스트로 옮김 
	else
	{
...
BOOL kEndTask(QWORD qwTaskID)
{
	...
	//현재 실행중인 태스크면 endtask비트를 설정하고 태스크를 전환
	pstTarget = gs_stScheduler.pstRunningTask;
	if (pstTarget->stLink.qwID == qwTaskID)
	{
		...
		//태스크가 전환되었으므ㄹ로 아래 코드는 절대 실행되지 않음
		while (1);
	}
	//실행준인 태스크가 아니면 준비 큐에서 직접 찾아서 대기 리스트에 연결
	else
	{
...
BOOL kIsTaskExist(QWORD qwID)
{
	...
	//tcb가 ㅇ벗거나 id가 일치하지 않으면 존재하지 않는 것임
...
//TCB풀에서 태스크 자료구조 추출
	...
	//만약 프로세스가 없거나 태스크 ID가 일치하지 ㅇ낳는다면 NULL을 반환
